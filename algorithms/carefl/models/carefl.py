...
class CAREFL:
    ...
    def flow_lr(self, data, return_scores=False):
        ...
        """
        dset, test_dset, _, dim, _ = self._get_datasets(data)
        self.dim = dim
        # Conditional Flow Model: X->Y
        torch.manual_seed(self.config.training.seed)
        flows_xy, _, _, _, _, _ = self._train(dset)
        self.flow_xy, score_xy, self._nlxy, self._nhxy, _, _, _, _, _, _ = self._evaluate(flows_xy, test_dset)
        # Conditional Flow Model: Y->X
        torch.manual_seed(self.config.training.seed)
        flows_yx, _, _, _, _, _ = self._train(dset, parity=True)
        self.flow_yx, score_yx, self._nlyx, self._nhyx, _, _, _, _, _, _ = self._evaluate(flows_yx, test_dset,
                                                                                          parity=True)
        # compute LR
        p = score_xy - score_yx
        self._update_dir(p)
    ...
    def _get_flow_arch(self, parity=False):
        ...
        elif self.config.flow.prior_dist == 'gaussian':
            prior = Normal(torch.zeros(dim).to(self.device), torch.ones(dim).to(self.device))
        elif self.config.flow.prior_dist == 'uniform_SigmoidTransform':  # looks like Gaussian
            prior = TransformedDistribution(Uniform(torch.zeros(dim).to(self.device), torch.ones(dim).to(self.device)),
                                            SigmoidTransform().inv)
        else:
            raise Exception("undefined prior distribution: {}".format(self.config.flow.prior_dist))
        ...
        def ar_flow(hidden_dim):
            if self.config.flow.architecture.lower() in ['cl', 'realnvp']:
                return AffineCL(dim=dim, nh=hidden_dim, scale_base=self.config.flow.scale_base,
                                ...
                                fix_AffineCL_forward=self.config.flow.fix_AffineCL_forward)
        ...
        for nl in self.n_layers:
            for nh in self.n_hidden:
                ...
                normalizing_flows.append(NormalizingFlowModel(prior, flow_list, self.config.flow.flow_SEM_type)
                                         .to(self.device))
    ...
    def _train(self, dset, parity=False):
        ...
        all_total_loss_vals = []
        all_prior0_logprob_vals = []
        all_prior1_logprob_vals = []
        all_log_det0_vals = []
        all_log_det1_vals = []
        for flow in flows:
            ...
            for e in range(self.epochs):
                total_loss_val = 0
                prior0_logprob_val = 0
                prior1_logprob_val = 0
                log_det0_val = 0
                log_det1_val = 0
                for _, x in enumerate(train_loader):
                    ...
                    if parity and self.config.flow.architecture == 'spline':
                        ...
                        x = x[:, [1, 0]]
                    # compute loss
                    _, prior_logprob, log_det0, log_det1 = flow(x)
                    total_loss = - torch.sum(prior_logprob.sum(dim=1) + log_det0 + log_det1)

                    with torch.no_grad():
                        prior0_logprob = prior_logprob[:, 0]
                        prior1_logprob = prior_logprob[:, 1]
                        total_loss_val += total_loss.item()
                        prior0_logprob_val += torch.sum(prior0_logprob).item()
                        prior1_logprob_val += torch.sum(prior1_logprob).item()
                        log_det0_val += torch.sum(log_det0).item()
                        log_det1_val += torch.sum(log_det1).item()
                ...
                if self.config.optim.scheduler:
                    scheduler.step(total_loss_val / len(train_loader))
                if self.verbose:
                    print('epoch {}/{} \tloss: {}'.format(e, self.epochs, total_loss_val))
                total_loss_vals.append(total_loss_val)
                prior0_logprob_vals.append(prior0_logprob_val)
                prior1_logprob_vals.append(prior1_logprob_val)
                log_det0_vals.append(log_det0_val)
                log_det1_vals.append(log_det1_val)
            all_total_loss_vals.append(total_loss_vals)
            all_prior0_logprob_vals.append(prior0_logprob_vals)
            all_prior1_logprob_vals.append(prior1_logprob_vals)
            all_log_det0_vals.append(log_det0_vals)
            all_log_det1_vals.append(log_det1_vals)
        return flows, all_total_loss_vals, all_prior0_logprob_vals, all_prior1_logprob_vals, all_log_det0_vals, all_log_det1_vals
    ...
    def _evaluate(self, flows, test_dset, parity=False):
        ...
        for idx, flow in enumerate(flows):
            if parity and self.config.flow.architecture == 'spline':
                raise Exception("fix the implementation later")
                # spline flows don't have parity option and should only be used with 2D numpy data:
                log_p_mean = np.nanmean(
                    np.concatenate([flow.log_likelihood(x.to(self.device)[:, [1, 0]])[0] for x in loader]))
            else:
                # log p(x0, x1), log p(x0) or log p(x0|x1), log p(x1) or log p(x1|x0), _, _, log p(z0), log p(z1)
                log_p_mean, log_p_x0_mean, log_p_x1_mean, _, _, log_p_z0_mean, log_p_z1_mean = np.nanmean(
                    np.concatenate([flow.log_likelihood(x.to(self.device)) for x in loader], axis=1), axis=1)

                # log_JacDet_x0, log_JacDet_x1
                # shape (N,)
                _, _, _, log_det0, log_det1, _, _ = np.concatenate(
                    [flow.log_likelihood(x.to(self.device)) for x in loader], axis=1)

            log_likelihood_means.append(log_p_mean)
            log_p_x0_means.append(log_p_x0_mean)
            log_p_x1_means.append(log_p_x1_mean)
            log_det0s.append(log_det0)
            log_det1s.append(log_det1)
            log_p_z0_means.append(log_p_z0_mean)
            log_p_z1_means.append(log_p_z1_mean)
        try:
            # in case all log_likelihood_means are nan, this will raise a ValueError
            idx = np.nanargmax(log_likelihood_means)
        except ValueError:
            # arbitrarily pick flows[0], this doesn't matter since best_score = nan, which will
            idx = 0

        if len(log_likelihood_means) != 1:
            raise Exception("What??? len(log_likelihood_means)={}".format(len(log_likelihood_means)))

        # unlike nanargmax, nanmax only raises a RuntimeWarning when all log_likelihood_means are nan, and will return nan
        best_score = np.nanmax(log_likelihood_means)
        best_flow = flows[idx]
        nl, nh = self._get_params_from_idx(idx)  # for debug
        return best_flow, best_score, nl, nh, log_p_x0_means[0], log_p_x1_means[0], log_det0s[0], log_det1s[0], \
               log_p_z0_means[0], log_p_z1_means[0]
    ...
    def _get_datasets(self, input):
        ...
        if isinstance(input, np.ndarray):
            ...
            train_dset_numpy = input.astype(np.float32)
            test_dset = CustomSyntheticDatasetDensity(data_test.astype(np.float32))
            test_dset_numpy = data_test.astype(np.float32)
            return dset, test_dset, test_dset_numpy, dim, train_dset_numpy
        if isinstance(input, Dataset):
            dim = input[0].shape[-1]
            return input, input, None, dim, None
        if isinstance(input, (tuple, list)):
            dim = input[0][0].shape[-1]
            return input[0], input[1], None, dim, None
